...
2. It must be possible to specify a command group of size `m` together with a maximum number of parallel processes `n`, where `n<m`. The commands in the group must be assigned to the available processes in the order in which they are declared in the project file.
...
Note that, with the second option, it is impossible to completely rule out losing `stdout` output when a subprocess is killed. This is because subprocesses manage their own `stdout` buffering and the contents of the buffer typically get lost when a subprocess dies. Although this effect can be avoided for Python commands by adding `-u` after the `sys.executable` to prevent buffering (see [example_queueing_with_output_management.py](https://github.com/richardpaulhudson/spacy_multiprocessing_arch/blob/main/example_queueing_with_output_management.py)), the same is not true of all other commands at the OS level. However, this is not necessarily a big problem because the console output for the command that **actually failed** and led to the other commands being killed — and this is the command that the user will usually want to investigate — will never get lost.
...
- they want to make sure output cannot be lost from non-Python commands in a group where another command fails
